...
async function LavalinkList(interaction, client, language) {
  ...
  for (let i = 0; i < client.lavalink_list.length; i++) {
    ...
    if (element.host == client.lavalink_using[0].host) {
      string_array.push(
        `\`${i + 1}: [USING] | ${element.online ? "🟢" : "🔴"} | ws://${
          element.host
        }:${element.port}/v3/websocket\``,
      );
    } else {
      string_array.push(
        `\`${i + 1}: ${element.online ? "🟢" : "🔴"} | ws://${element.host}:${
          element.port
        }/v3/websocket\``,
      );
    }
  ...
  if (pages.length == pagesNum && string_array.length > 15)
    SlashPage(client, interaction, pages, 60000, string_array.length, language);
