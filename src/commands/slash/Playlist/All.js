...
module.exports = {
    ...
    run: async (interaction, client, language) => {
        ...
        Object.keys(fullList).filter(function(key) {
            return fullList[key].owner == interaction.user.id;
        }).forEach(async (key, index) => { playlists.push(fullList[key]) })
        ...
        if (!number) {
            if (pages.length == pagesNum && playlists.length > 10) {
                SlashPlaylist(client, interaction, pages, 30000, playlists.length, language);
