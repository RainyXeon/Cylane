...
module.exports = {
    ...
    run: async (client, message, args, language, prefix) => {
        ...
        if (!message.guild.members.cache.get(client.user.id).permissionsIn(channel).has(PermissionsBitField.Flags.Connect)) return message.channel.send(`${client.i18n.get(language, "playlist", "import_join")}`);
        if (!message.guild.members.cache.get(client.user.id).permissionsIn(channel).has(PermissionsBitField.Flags.Speak)) return message.channel.send(`${client.i18n.get(language, "playlist", "import_speak")}`);
        ...
        if (!playlist) return message.deferReply(`${client.i18n.get(language, "playlist", "invalid")}`)
        ...
        for (let i = 0; i < playlist.tracks.length; i++) {
            const res = await player.search(playlist.tracks[i].uri, { requester: message.author });
            if(res.type == "TRACK") {
