    ...
    run: async (client, message, args, language, prefix) => {
        ...
        const db = await client.db.get(`control.guild_${message.guild.id}`)
            ...
            .setDescription(`${client.i18n.get(language, "utilities", "control_set", {
                toggle: db == "enable" ? `${client.i18n.get(language, "music", "disabled")}` : `${client.i18n.get(language, "music", "enabled")}`
            })}`)
            .setColor(client.color)

        message.channel.send({ embeds: [embed] });
        return client.db.set(`control.guild_${message.guild.id}`, db.enable == "enable" ? "disable" : "enable")
    }
}