    ...
    run: async (client, message, args, language, prefix) => {
        ...
        if(value && !isNaN(value)) {
            if((song_position - value * 1000) > 0) {
                await player.send({ op: "seek", guildId: message.guild.id, position: song_position - value * 1000 });
        ...
        if(!value) {
            if((song_position - rewindNum * 1000) > 0) {
                await player.send({ op: "seek", guildId: message.guild.id, position: song_position - rewindNum * 1000 });
