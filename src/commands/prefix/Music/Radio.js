...
module.exports = { 
    ...
    run: async (client, message, args, language, prefix) => {
        ...
        const msg = await message.channel.send(`${client.i18n.get(language, "music", "radio_loading")}`);

        const value = args[0]
        if (value && isNaN(value)) return msg.edit(`${client.i18n.get(language, "music", "number_invalid")}`);

        const { channel } = message.member.voice;
        if (!channel) return msg.edit(`${client.i18n.get(language, "music", "search_invoice")}`);
        if (!message.guild.members.cache.get(client.user.id).permissions.has(PermissionsBitField.Flags.Connect)) return msg.edit(`${client.i18n.get(language, "music", "radio_join")}`);
        if (!message.guild.members.cache.get(client.user.id).permissions.has(PermissionsBitField.Flags.Speak)) return msg.edit(`${client.i18n.get(language, "music", "radio_speak")}`);
